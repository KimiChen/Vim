...
function! <SID>StartExplorer(sticky, delBufNum)
  ...
  if has("syntax")
    syn clear
    syn match MBENormal             '.*:\([0-9]*\).*'
    syn match MBEChanged            '+.*:\([0-9]*\).*'
    syn match MBEVisibleNormal      '.*:\([0-9]*\).*\*+\='
    syn match MBEVisibleChanged     '.*:\([0-9]*\).*\*+'
    
    if !exists("g:did_minibufexplorer_syntax_inits")
      let g:did_minibufexplorer_syntax_inits = 1
      hi def link MBENormal         Comment
      hi def link MBEChanged        DiffChange
      hi def link MBEVisibleNormal  Define
      hi def link MBEVisibleChanged Define
    endif
  endif
